   ...
   // form.hostname = location.origin;
    ...
    .subscribe((response:any)=> {
      if(response.status === -1 || response.status === 1){
        ...
        debugger;
        if(this.ticket.userType==0){ this.router.navigate(['/user'])}
         if(this.ticket.userType==2){ this.router.navigate(['/lab-path'])}
         if(this.ticket.userType==3){ this.router.navigate(['/reception'])}
         if(this.ticket.userType==4 || this.ticket.userType==44 || this.ticket.userType==444 ){ this.router.navigate(['/lab-rad'])}
         if(this.ticket.userType==5){ this.router.navigate(['/indoor'])}
         if(this.ticket.userType==6){ this.router.navigate(['/obs-reg'])}
         if(this.ticket.userType==7){ this.router.navigate(['/ot'])}
         if(this.ticket.userType==8){ this.router.navigate(['/pharmacy'])}
         if(this.ticket.userType==9){ this.router.navigate(['/community'])}
         if(this.ticket.userType==10){ this.router.navigate(['/fac-staff'])}
         if(this.ticket.userType==11){ this.router.navigate(['/rc-point'])}
         if(this.ticket.userType==12){ this.router.navigate(['/hiv'])}else{this.router.navigate(['/login'])}
         alert('non correct')
