...
getDiagnosis() {
  this.userLoader= true;
  this.modelInvest.hospitalID=this.hospitalID;
  this.modelInvest.prescriptionID=this.hospitalID;
  // this.model.tokenID=this.otDetails.ptID;
  
 console.log('modelInvest ==', this.modelInvest);
  this.otServices.getInvestigations(this.modelInvest).subscribe(
    (response: any) => {
      if (response.status === 0) {
