    ...
    (response: any) => {
      if (response.status === 0) {
       ...
       for(let i=0;i<=this.newArray.length ;i++){
         if(this.newArray && this.newArray[i].stock!=undefined){
          console.log('newwwwww===',this.newArray[i].stock)
         }
        
        
       }

    //   if(this.newArray[1].stock.length!=0){
    //   this.newArray[1].stock.forEach(element => {
...
getdata(dataobj){
  debugger
  console.log('batch data ==',dataobj.target.value);
