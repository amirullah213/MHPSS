...
export class SmrPatDetailComponent implements OnInit {
  ...
  openModAdd3(captureuser3: TemplateRef<any>) {
   ...
   childVacination(modData) {
    ...
    this.pharmaService.generToken(this.model4).subscribe(
     (response: any) => {
  ...
  ttvacination(modData) {
    ...
    this.pharmaService.generToken(this.model3).subscribe(
     (response: any) => {
       if (response.status === 0) {
 ...
 getAllDoctros() { 
  ...
  this.pharmaService.getDoctorsList(this.model1).subscribe(
    (response: any) => {
      if (response.status === 0) {
...
updateclinic(modData) {
  ...
  this.pharmaService.updateclinic(this.model21).subscribe(
