...
export class SmrPatDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    console.log('itemmmmid',this.itemid)
 ...
 getDispenseDetail() { 
   ...
   this.model1.itemID=this.itemid.itemID;
   this.model1.pharmacyID=this.pharmacyType.id;
   this.model1.date=formatDate(this.smrpatdata.DateCreated, 'yyyy-MM-dd', 'en');
  ...
  this.pharmaService.getDispenseDetail(this.model1).subscribe(
    (response: any) => {
      if (response.status === 0) {
         ...
         this.docResponseArray1=response;
         console.log("this.docResponseArray1===",this.docResponseArray1.patients.length);
  ...
  gotoPrintPrescription(patdata){
    debugger
    if(patdata.isIndoor==0){
      localStorage.setItem('pharmacyData',JSON.stringify(patdata) );
      window.open('/print/home')
    }else{
      localStorage.setItem('wardData',JSON.stringify(patdata) );
      window.open('/print/discharge-card')
    }
