...
saveData(dat){
  ...
  updateToken(){
    ...
    this.loader_eqp = true;
    this.modal34.hospitalID=this.hospitalID;
    this.modal34.isPrescribed=2;
     this.modal34.ptID=this.padata.ptID;
    ;
  
  
      console.log('this.modal34===',this.modal34); 
      
     this.pharmacySer.updatePatientToken(this.modal34).subscribe(
        (response: any) => {
          if (response.status === 0) {
