...
onSubmit() {
  ...
  openModalWithClass(template: TemplateRef<any>,data) {
    ...
    debugger
    if(this.userData.subTests!=null){
...
//for test types data
       ...
       if(!this.pathArrNew_added){
        ...
        response.testData.forEach(mm => {
          if(mm.isDirect==1){
          this.pathArrNew.push({"id":-1,"result":"","patientID":this.detailsData.patientID,"testName":mm.testName,"testID":mm.testID,"testType":mm.testType,"refRange":mm.refRange,"isSupper":mm.isSupper,"isDirect":mm.isDirect,"subTests":mm.subTests?mm.subTests:null,"xrayFilms6":mm.xrayFilms6})
        }
        });
        ...
        response.testData.forEach(k => {
          if(k.isDirect==1){
          this.pathArrNew.push({"id":-1,"result":"","patientID":this.detailsData.patientID,"testName":k.testName,"testID":k.testID,"testType":k.testType,"refRange":k.refRange,"isSupper":k.isSupper,"isDirect":k.isDirect,"subTests":k.subTests?k.subTests:null,"xrayFilms6":k.xrayFilms6})
          }
        });
        }
