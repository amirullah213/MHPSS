...
// }
  ...
  getUserList() {
    ...
    this.model.user_id  =  this.detailsData.id;

    this.adminService.get_mhpss_sessions_by_user(this.model).subscribe(
      (response: any) => {
  ...
  gotoShowDetails(udata){
    debugger
    localStorage.setItem('showData',JSON.stringify(udata) );
    this.router.navigate(['/admin/showData']);
  ...
  getSessionImages() {
    ...
    //  this.model.auth_token =  localStorage.getItem('auth_token');
    //  console.log('test==',this.model)
    this.model.session_id  =  this.detailsData.id;
