...
this.model.testType=1;
 ...
 addnewTest(objmed) {
    ...
    //  console.log('local storage==',localStorage.getItem('auth_token'));
      this.model.isSupper = objmed.isSupper;
      this.model.testName = objmed.name;
      this.model.refRange = objmed.range;
      this.model.fee = objmed.fee;
      this.model.subTests = this.attsArr;
      
     
    //  console.log('test==',this.model)

    this.adminService.addTest(this.model).subscribe(
      (response: any) => {
        if (response.status === 0) {
            this.loader_eqp = false;
            this.getAllPathList();
