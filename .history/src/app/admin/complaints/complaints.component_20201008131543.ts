...
export class ComplaintsComponent implements OnInit {
  ...
  showEditData(edtObj){
   ...
   getAllComp() {
    ...
    this.adminService.allComplaints(this.model).subscribe(
     
      (response: any) => {
        if (response.status === 0) {
          this.allComplaints = response.data;
          console.log('all allComplaints==', this.allComplaints);
 ...
 addNewCom(objmed) {
  ...
  //get all user list
   ...
   updateComp(objmed) {
    ...
    this.adminService.updateSign(this.model2).subscribe(
      (response: any) => {
        if (response.status === 0) {
         // this.allUsers = response.data;
         //this.dtTrigger.next();
