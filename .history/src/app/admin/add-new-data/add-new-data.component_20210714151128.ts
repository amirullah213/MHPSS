...
this.treatmentForm.patchValue({
  team_member:this.detailsData.name
})  //------------
  }
...
add_mhpss_session() {
 ...
 if(AllFormsObj.village_id=='-1'){
  AllFormsObj.district= AllFormsObj.child.district;
  AllFormsObj.taluka= AllFormsObj.child.taluka;
  AllFormsObj.union_council= AllFormsObj.child.union_council;
  AllFormsObj.village= AllFormsObj.child.village;
 
 }
 if(AllFormsObj.lhw=="Others"){
  AllFormsObj.lhw=AllFormsObj.lhwOther;
 }
 if(AllFormsObj.lhs=="Others"){
  AllFormsObj.lhs=AllFormsObj.lhsOther;
 }
 if(AllFormsObj.cap=="Others"){
  AllFormsObj.cap=AllFormsObj.capOther;
 }
  AllFormsObj.user_id=this.detailsData.id;
  AllFormsObj.photos=this.imageArr;
  AllFormsObj.photo_of_attd_sheet=this.imageUrlSheetArr;
      ...
      //     if (response.status === 0) {
        debugger
           ...
           console.log('allList==',this.allList);
            ...
            console.log('allListurl==',this.imageUrl);
           // this.imageArrSheet=
           this.imageUrlSheetArr.push(this.imageUrlSheet);
