...
@Component({
  selector: 'ncri-seen-patients',
  templateUrl: './seen-patients.component.html',
  styleUrls: ['./seen-patients.component.scss']
})
export class SeenPatientsComponent implements OnInit {
  ...
  this.otServices.getOperationTheatre(this.model).subscribe(
    (response: any) => {
      if (response.status === 0) {
        ...
        console.log('this.optheater data==',this.PathResponseArray);
        this.diagArrOT=JSON.parse(this.PathResponseArray.diagnosis);
        console.log('this.optheater diagArrOT==',this.diagArrOT);
...
generatetoken() {
  ...
  this.modalToken.patientID=this.otDetails.patientID;

  this.modalToken.departmentID=this.deptID;
